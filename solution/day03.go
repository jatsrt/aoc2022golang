...
func Day03Solution(ctx context.Context, day int, input []byte) {
	...
	common := funk.Map(compartments, func(x [2][]rune) int32 { return intersect.Simple(x[0], x[1])[0].(int32) }).([]int32)
	solution := funk.Reduce(common, func(acc int32, c int32) int32 { return char_val(c) + acc }, 0).(int32)

	log.Ctx(ctx).Info().Int("part", 1).Int32("solution", solution).Msg("solved")

	solution = funk.Reduce(funk.Chunk(sacks, 3), func(acc int32, group []string) int32 {
		return char_val(int32(intersect.Simple(group[0], intersect.Simple(group[1], group[2]))[0].(uint8))) + acc
	}, 0).(int32)

	log.Ctx(ctx).Info().Int("part", 2).Int32("solution", solution).Msg("solved")
}
